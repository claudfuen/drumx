...
The port uses the same 20 authored patterns and 80 original Big Rusty recordings as the Mac lab. Shared JSON comes from the Swift course exporter. The extension owns MIDI capture, scoring, sample playback, and the click; Godot displays their results. Full-kit MIDI monitoring includes toms, crash, ride, and separate open/pedal hi-hats; the shared curriculum still scores hi-hat, snare, and kick. The [native backend guide](../apps/game/native/README.md) records its clock, queue, and device boundaries.
