...
A publishing job then creates **`preview-<12-character commit SHA>`** and an **experimental prerelease** bound to the full verified commit. It runs only on trusted main-branch pushes or manual main-branch workflow runs in this repository. Pull requests build and test without publishing or receiving signing secrets. Write permission is confined to the release job.

The release includes Mac and Windows ZIPs, `build-manifest.json`, `SHA256SUMS.txt`, and the three project license files. The publisher stages uploads, verifies their hashes, and automatically publishes the completed prerelease. It never moves an existing tag or overwrites a different asset. A rerun with the original artifacts is idempotent; rebuilding a released commit may produce different bytes and is rejected rather than replacing history.
