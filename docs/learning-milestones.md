...
The current macOS build connects welcome and local player profiles to a 20-step foundation path, visual kit setup, shared notation/audio/scoring events, reading checks, technique self-checks, assistance, review, and versioned resume. New lessons use a 16-bar practice block with detailed options initially collapsed. These are M1 implementation progress, not evidence that a real beginner has completed the intended journey successfully. See [current behavior](native-lab.md), [authored course](../native/macos/DrumxCourse.swift), and the [authored curriculum and expansion gate](curriculum-review.md). This document owns definitions and milestone acceptance; the lesson guide owns implementation details.
...
The implemented course keeps **Practised**, **Reading checked**, and **Recall tried** as separate evidence. A correct reading response and explicit technique self-check are different records. The first completed pulse attempt is a baseline comparison, not automatic skill placement. Scheduled revisits and skill placement remain planned. Coached pulse and repeated per-instrument pattern checkpoints are implemented.

The lesson path uses explicit pattern checkpoints. Find the pulse has the [72 BPM coached checkpoint](guided-tempo.md). Other lessons require two qualifying takes within three comparable attempts at the authored tempo, 16 bars, and Guided notes. Each required instrument needs 80% matched and 70% on time, with extras no greater than 10% of expected targets. Chapter transitions also require the preceding final reading check. These are access rules, not broad readiness or mastery. Old access is preserved separately from new checkpoint evidence; later difficult practice does not erase an earned checkpoint, while corrected results are reevaluated. Learn's stars show the best current-version result across any conditions; review compares matching conditions. [Current curriculum review](curriculum-review.md) explains the failure modes this policy addresses.
...
Timing windows and numerical star boundaries are **prototype score policy**, not learner gates. The implemented pulse and per-instrument checkpoint rules, and future learning cutoffs, repetition counts, tempo targets, or revisit intervals, are **untested starting hypotheses** until evaluated with learners. Keep their meaning explicit when versioning content or policy. No research-backed mastery percentage or automatic certification is specified here.
...
The [guided-tempo implementation](guided-tempo.md) provides authored pulse starting/checkpoint/challenge paces, explained recommendations from repeated comparable evidence, and condition-labeled control/recall achievements. Other lessons have authored starting tempos and fixed checkpoint conditions with a direct setup action; they do not yet have adaptive tempo ladders. All paces and thresholds remain hypotheses to test. Shared-port visual acceptance remains open.
...
| --- | --- | --- |
| Coherent foundation unit | Twenty authored lessons in five chapters cover pulse/counts, a layered backbeat, rudiments, and kit applications. Original 12 event identities are regression protected; all20 are checked against notation, scoring, and demonstration events. The new sticking strip has offscreen component evidence, not whole-app visual acceptance. | Observe learner comprehension and the musical experience on a physical kit. |
...
- **Production engine and portability:** choose the renderer/engine before large content expansion and validate a representative complete journey. The current custom AppKit shell is Mac-only. The C++ core and C consumer passed hosted Debug/Release contracts on both operating systems, plus Mac sanitizers; see [verified platform evidence](cross-platform.md#what-has-been-verified). The experimental shared Godot app now consumes verified course JSON and native Mac/Windows input/audio adapters, with paired exported-app CI. The first port failed visual review, so visual parity and physical-device acceptance remain explicit gates. Successful executable checks do not establish presentation quality or measured frame performance.
