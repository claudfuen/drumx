...
The chapter journey also has a prototype access rule: finish at least four bars with 80% of expected notes matched to clear a step; entering the next chapter additionally requires the preceding chapter's final reading check. All chapters remain inspectable, while locked lesson cards explain their prerequisite and cannot start. Existing saved positions and earlier practice preserve access; old revisions do not pass new checks. **Step complete** and **lessons cleared** describe this game progression rule, not broader readiness or mastery. Stars, tighter on-time grades, recall, and technique remain separate. See [menu architecture](menu-architecture.md#unlocks-invite-another-step) for the current route and policy boundaries.
...
| Understand, practice, review | Lesson objectives, counts, three-choice reading questions, explicit technique self-checks, scores/PBs, and explained lesson unlocks are available. Estimated repetition time totals 96 suggested minutes, not recorded-content duration. | Observe a beginner understanding the task, interpreting feedback, and choosing a useful next action without developer intervention. |
...
| Reliability and experience | Native checks cover custom charts, shared course audio, local profiles, durable archives, delayed corrections, and unlock sequencing. Main menu, chapter journey, settings controls/persistence, pause, and locked review states were inspected in the app. | Real-kit reliability, physical latency and sustained frame measurements, broader interruption/accessibility coverage, and the full finished-game experience bar remain open. |
...
- **Production engine and portability:** choose the renderer/engine before large content expansion and validate a representative complete journey. The current custom AppKit shell is Mac-only. The C++ scoring core is portable; Swift lesson/progression models need extraction or porting, and Windows needs input/audio layers. Custom styling does not establish shared UI portability or measured frame performance.
