...
| --- | --- | --- |
| Shared foundations and coached pulse | Both source and exported applications pass 10,781 checks at `9ddcab4` in [paired CI](https://github.com/claudfuen/drumx/actions/runs/34728349618), including the 20-lesson course and per-instrument checkpoints. | Observe the complete practice journey. |
| Native Mac models and controls | The full native suite passes locally. CI passes 446 history, 57 tempo adapter, 194 readiness, and 99 settings/course-control checks and compiles the native Mac app. [Control detail](images/settings-controls-detail.png) was rendered directly from native views into a bitmap and inspected. It is a component fixture, not an app screenshot. | Inspect the complete app interactively. |
...
| Distribution integrity | Both packages use clean commit `9ddcab4`; CI verifies archive hashes, assets, licenses, and exported execution, then publishes the immutable experimental tag. Both public downloads passed 48 integrity/license checks; the downloaded Mac executable passed all 10,781 checks. [Release manifest](https://github.com/claudfuen/drumx/releases/download/preview-9ddcab48cd71/build-manifest.json). | Player download/launch and visual acceptance remain required before stable release. |
| Complete rendered journey | **Pending.** Offscreen notation, menu, gameplay, and settings components have been inspected; the updated native app was opened locally. These do not constitute full journey acceptance. | Complete interactive window review and physical Windows graphics/scaling. |
